"""Run hierarchical retrieval, navigation, promotion, and answer generation."""
...
class Embedder:
    def __init__(self, model_name: str = EMBED_MODEL):
        ...
        if backend in ("azure_large", "azure_small"):
            ...
            _dim = 1536 if backend == "azure_small" else 3072
            log.info("loading AzureEmbedder (%s, dim=%d) ...", backend, _dim)
...
def _load_te3_index_once():
    ...
    cache = _Path_te3(
        os.environ.get(
            ...
            str(_Path_te3(REPO_ROOT) / "results/v5_emb_large_llm_distill/erag_60M_gold/_index_cache"),
        )
    )
